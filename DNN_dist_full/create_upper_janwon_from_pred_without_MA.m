...
function create_upper_janwon_from_pred_without_MA(sub)
...
%         elseif strcmp(sub{s}, 'M2'), c3_st_idx = contour3_start(4); end
    ...
    elseif strcmp(sub{s}, 'M2'), hard_palate_sub = hard_palate{4}; end
     ...
     for fold = 1:4
        ...
        file_path = ['./Upper_contours_dnn_without_MA/' sub{s} '/Fold' num2str(fold)];
	mkdir(file_path)
        upper_contour_smooth = struct;
        ...
        load(['./Upper/' sub{s} '/Fold' num2str(fold) '/Pred_Set.mat'],'Pred_Set');
        
        %load([file_path '/fold' num2str(fold) '_janwon_' sub{s} '_annotation.mat']);
        
        for f = 1:length(fold_str)
            %vel_idx = nearestneighbour(annotation_for_janwon(f).VEL', Pred_Cont_GridWise_MA{1,f}');
%             vel_search = length(Pred_Set{1,f}) - 10;
%             [~, vel_idx] = max(Pred_Set{1,f}(vel_search:end, 2));
%             vel_idx = vel_search - 1 + vel_idx;
            hard_idx = nearestneighbour(hard_palate_sub(5,:)',Pred_Set{1,f}');
            [~, vel_idx] = max(Pred_Set{1,f}(hard_idx:end, 2));
            vel_idx = vel_idx + hard_idx - 1;
            c3_st_idx = find(contour3(:, 2) > Pred_Set{1,f}(vel_idx, 2), 1, 'first');
            %c3_end_idx = nearestneighbour(fold_str(f).c_up(end, :)', annotation_for_janwon(f).contour3');
            c3_end_idx = GLTBpt;
            st_idx = nearestneighbour(fold_str(f).c_up(1,:)', Pred_Set{1,f}');
...
%             hold off;
            ...
            upper_contour_smooth(f).upper_contour = [Pred_Set{1,f}(st_idx:vel_idx, :); contour3(c3_st_idx:c3_end_idx, :)];
            upper_contour_smooth(f).upper_contour_without_c3 = [Pred_Set{1,f}(st_idx:vel_idx, :)];
