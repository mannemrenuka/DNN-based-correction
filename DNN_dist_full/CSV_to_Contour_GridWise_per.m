...
%              n1 = n/2;
             ...
             M = n*csvread(['./'  Segment  '/' strsub{sub} '/' strfold{fold} '/Pred_Data_Grid_' num2str(per(perc)) '.csv']);
             Pred_Set = {};
             for i = 1:np+1
...
%                  v = reshape(M(i,:),[pt,length(M)/pt]);
                 ...
                 for frames = l-l1+1:l
                     ...
                     v = v1(1,c:pt) + (i-1)*del;
                     for p = 1:s(1)
                         x_c = Dev_Complete_Data{frames,1}(p,:);
                         y_c = Dev_Complete_Data{frames,2}(p,:);
                         pred_cont{frames-l+l1}(p,:) = cord_line([x_c(1),y_c(1)],[x_c(end),y_c(end)],v(1,p));
                     end
                     c = pt + 1;
                     if frames ~= l
