...
for perc = 1:length(per)
    for sub=1:length(strsub)
         ...
         for fold=1:length(strfold)
             ...
             load(['./'  Segment  '/' strsub{sub} '/' strfold{fold} '/Dev_Complete_Data_' num2str(per(perc)) '.mat'],'Dev_Complete_Data');
             ...
             for frame=1:length(Dev_Complete_Data)
                 ...
                 for p=1:length(Dev_Complete_Data{frame,3}(:,1))
                     ...
                     for t=1:length(seqt(:,1))
                         if flag~=1
                             x_c = Dev_Complete_Data{frame,1}(p,seqt(t,:));
                             y_c = Dev_Complete_Data{frame,2}(p,seqt(t,:));
                             nind = (pdist2([x_c(1),y_c(1)],[xy(1),xy(2)]))/(pdist2([x_c(1),y_c(1)],[x_c(end),y_c(end)]));
                             intensity_profile_set{p,t}=[Dev_Complete_Data{frame,3}(p,seqt(t,:)) nind];
