...
for perc = 1:length(per)
     for sub=1:length(strsub)
        ...
        for fold=1:length(strfold)
            ...
            load(['./DNN_Data_dist_DNN_Full/' Segment  '/' strsub{sub} '/' strfold{fold} '/distance_dev.mat'],'dist_hist_dev');
            load([filepath 'Autorun_Ton1/' strsub{sub} '/' strfold{fold} '/Dev.mat']);
            load([ './Autorun/Upper/' strsub{sub} '/' strfold{fold} '/full_smooth_dev.mat'], 'full_smooth_dev');
            ...
            save(['./DNN_Data_dist_DNN_Full/'  Segment  '/' strsub{sub} '/' strfold{fold} '/Dev_Complete_Data_' num2str(per(perc)) '.mat'],'Dev_Complete_Data');
        end
        save(['./DNN_Data_dist_DNN_Full/'  Segment  '/' strsub{sub} '/pxl_np_' num2str(per(perc)) '.mat'],'pxl_np');
     end
end
