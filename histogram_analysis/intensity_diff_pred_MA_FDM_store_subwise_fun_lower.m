...
filepath3 = '/home/spire/Documents/DNN_journal_16videos/Autorun_dist_DNN_SegNet/Autorun/';
 for sub=1:length(strsub)
     ...
     for fold=1:length(strfold)
         ...
         load([ filepath 'Lower_contours/' strsub{sub} '/' strfold{fold} '/Lower_' strsub{sub} '_fold' num2str(fold)  '_sm.mat'], 'lower_contour_smooth');
         fdm = lower_contour_smooth;
         clear lower_contour_smooth;
         load([ filepath2 'Lower_contours/' strsub{sub} '/' strfold{fold} '/Lower_' strsub{sub} '_fold' num2str(fold)  '_sm.mat'], 'lower_contour_smooth');
         ma = lower_contour_smooth;
         clear lower_contour_smooth;
         load([ filepath3 'Lower/' strsub{sub} '/' strfold{fold} '/Lower_' strsub{sub} '_fold' num2str(fold)  '_sm.mat'], 'lower_contour_smooth');
         seg = lower_contour_smooth;
         clear lower_contour_smooth;
         ...
         for frame=1:length(Test_Complete_Data_lower_pxl_5)
             for p=1:length(Test_Complete_Data_lower_pxl_5{frame,3}(:,1))
                ...
                gridpoints=[x_c;y_c];
                xy_f = hfn_interX(fdm(frame).contour_smooth',gridpoints);
                if ~isempty(xy_f)
...
%                     end
                ...
                xy_m = hfn_interX(ma(frame).contour_smooth',gridpoints);
                if ~isempty(xy_m)
...
%                     end
                ...
                xy_s = hfn_interX(seg(frame).contour_smooth',gridpoints);
                if ~isempty(xy_s)
%                     nind_m = ceil((pdist2([x_c(1),y_c(1)],[xy_m(1),xy_m(2)]))/thre);
%                     if nind_m-1~=0 && nind_m+2<=10
%                         diff_sub_ma = [diff_sub_ma ins(nind_m)-ins(nind_m+1) ins(nind_m-1)-ins(nind_m+1) ins(nind_m)-ins(nind_m+2)];
%                     end
     ...
     save(['./'  Segment '/' strsub{sub} '/diff_sub_fdm_lower_norm.mat'],'diff_sub_fdm');
     save(['./'  Segment '/' strsub{sub} '/diff_sub_ma_lower_norm.mat'],'diff_sub_ma');
     save(['./'  Segment '/' strsub{sub} '/diff_sub_seg_lower_norm.mat'],'diff_sub_seg');
     save(['./'  Segment '/' strsub{sub} '/diff_sub_grd_lower_norm.mat'],'diff_sub_grd');
 end