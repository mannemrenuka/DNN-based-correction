...
function create_lower_janwon_from_pred(sub)
...
for s = 1:size(sub, 2)
    for fold = 1:4
        ...
        load(['./Lower/' sub{s} '/Fold' num2str(fold) '/Pred_Cont_GridWise_MA.mat'],'Pred_Cont_GridWise_MA');
        %load(['Lower_contours/' sub{s} '/Fold' num2str(fold) '/Lower_' sub{s} '_fold' num2str(fold) '.mat']);
        file_path = ['./Lower_contours_dnn/' sub{s} '/Fold' num2str(fold)];
        mkdir(file_path);
        ...
        for f = 1:length(fold_str)
            ...
            st_idx = nearestneighbour(fold_str(f).c_down(1,:)', Pred_Cont_GridWise_MA{1,f}');
            end_idx = nearestneighbour(fold_str(f).c_down(end,:)', Pred_Cont_GridWise_MA{1,f}');
...
%             hold off;
            ...
            lower_contour_smooth(f).contour_smooth = make_janwon_contours(Pred_Cont_GridWise_MA{1,f}(st_idx:end_idx, :), Test(f).Frame);
            imagesc(fold_str(f).Frame); hold on;
            plot(fold_str(f).c_down(:, 1), fold_str(f).c_down(:, 2), 'g');
            plot(Pred_Cont_GridWise_MA{1,f}(:,1), Pred_Cont_GridWise_MA{1,f}(:,2),'b');
            plot(lower_contour_smooth(f).contour_smooth(:, 1), lower_contour_smooth(f).contour_smooth(:, 2), 'r');
            pause(0.001);
            hold off;
        end
        save(['./Lower_contours_dnn/' sub{s} '/Fold' num2str(fold) '/Lower_' sub{s} '_fold' num2str(fold) '_sm.mat'], 'lower_contour_smooth');
